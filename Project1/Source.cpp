...
main(int argc, char *argv[])
{
	...
	for (int NUMTRIALS = 10; NUMTRIALS <= 1000000; NUMTRIALS *= 10) {
		for (int i = 1; i <= 8; i = i * 2) {

			omp_set_num_threads(i);	// set the number of threads to use in the for-loop:`
...
#pragma omp parallel for default(none) shared(xcs,ycs,rs) reduction(+:numHits)
				for (int n = 0; n < NUMTRIALS; n++)
				{
					...
					float nx = xcir - xc;
					float ny = ycir - yc;
					float n_dis = sqrt(nx*nx + ny * ny);
					nx /= n_dis;	// unit vector
					ny /= n_dis;	// unit vector
			...
			printf("%d\t%d\t%5.2lf\t%5.2lf\n", i, NUMTRIALS, currentProb, maxPerformance);
			/*
			printf("The Total Threads is %d\n", NUMT);
			printf("The number of Trials is %d\n", NUMTRIALS);
			printf("The Probability of Hit hte plate is %5.2lf\n", currentProb);
			printf("The Max performance is %5.2lf\n", maxPerformance);
			*/
#pragma omp barrier
		}
	}
