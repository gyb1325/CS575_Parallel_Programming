...
#pragma omp parallel for default(none), shared(fullTileArea, halfTileArea, quatTileArea) reduction(+:sum)
			for (int i = 0; i < NUMNODES*NUMNODES; i++)
			{
				int iu = i % NUMNODES;
				int iv = i / NUMNODES;
				float temp_height = Height(iu, iv);
		...
		printf("the area is %5.2lf, max_performance is %5.2lf\n", sum, maxPerformance);
