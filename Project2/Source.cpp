...
int main(int argc, char *argv[])
{
	...
	return 1;
	#endif
	for (int NUMNODES = 40; NUMNODES <= 80; NUMNODES += 5) {
		for (int NUMT = 1; NUMT <= 8; NUMT *= 2) {
			omp_set_num_threads(NUMT);
			// the area of a single full-sized tile:
			float fullTileArea = (((XMAX - XMIN) / (float)(NUMNODES - 1))  *
				((YMAX - YMIN) / (float)(NUMNODES - 1)));
			float halfTileArea = fullTileArea / 2.0;
			float quatTileArea = fullTileArea / 2.0;
...
#pragma omp parallel for default(none), shared(fullTileArea, halfTileArea, quatTileArea,NUMNODES ) reduction(+:sum)
				for (int i = 0; i < NUMNODES*NUMNODES; i++)
				{
					int iu = i % NUMNODES;
					int iv = i / NUMNODES;
					float temp_height = Height(iu, iv, NUMNODES);
					if ((iu == 0 && iv == 0) || (iu == NUMNODES - 1 && iv == NUMNODES - 1) || (iu == NUMNODES - 1 && iv == 0) || (iu == 0 && iv == NUMNODES - 1)) {
						sum += quatTileArea * temp_height;
					}
					else if ((iu == 0) || (iu == NUMNODES - 1) || (iv == 0) || (iv == NUMNODES - 1)) {
						sum += halfTileArea * temp_height;
					}
					else {
						sum += fullTileArea * temp_height;
					}

				}
				double time1 = omp_get_wtime();
				double megaOpsPerSecond = (double)NUMNODES*NUMNODES / (time1 - time0) / 1000000.;
				if (megaOpsPerSecond > maxPerformance)
					maxPerformance = megaOpsPerSecond;
			}
			printf("%d \t %d \t %5.4lf \t %5.4lf\n",NUMT, NUMNODES, sum, maxPerformance);
